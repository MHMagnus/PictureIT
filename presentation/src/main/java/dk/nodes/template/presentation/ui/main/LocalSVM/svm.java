package dk.nodes.template.presentation.ui.main.LocalSVM;
...
//
class Solver {
	int active_size; 	// The number of active examples with 0 < alpha[i] < get_C(i)
	byte[] y;			// The (integer) label \(\pm 1\) of the training example
	double[] G;			// gradient of objective function
	static final byte LOWER_BOUND = 0;
	static final byte UPPER_BOUND = 1;
	static final byte FREE = 2;
	byte[] alpha_status;// LOWER_BOUND, UPPER_BOUND, FREE, The status of each example
	double[] alpha;		// The weight \(\alpha\) of the Support Vectors
	QMatrix Q;
	double[] QD;		// Q-MATRIX is derived from kernel matrix: Q_{ij}=y_{i}*y_{j}*K_{ij}
	double eps;			// Tolerance of termination criterion
	double Cp,Cn;		// The regularization parameter of negative examples - Cp and negative examples - Cn
	double[] p;
	int[] active_set;
	double[] G_bar;		// gradient, if we treat free variables as 0
	int l;				// Total number of Support Vectors
	boolean unshrink;	// XXX
	public static double WAbsVal; // absolute value of w to calculate distance
	...
	{
		Log.i("home", "svm.java: Solve method was called");
		this.l = l;
		this.Q = Q;
		QD = Q.get_QD();
		p = (double[])p_.clone();
		...
		// calculate objective value
		{
			double v = 0;
			int i;
			Log.i("home", "svm.java: l = " + l);
			Log.i("home", "svm.java: alpha[0] = " + alpha[0]);
			for(i=0;i<l;i++){
				v += alpha[i] * (G[i] + p[i]);
//				Log.i("home", "svm.java: G[i]" + G[i]);
//				Log.i("home", "svm.java: p[i]" + p[i]);
				sumAlpha =  sumAlpha + alpha[i];
//				Log.i("home", "svm.java: alpha is incremented by " + alpha[i]);
//				Log.i("home", "svm.java: v value: " + v);
			}
				//v += alpha[i] * (G[i] + p[i]);

			si.obj = v/2; // variable for the objective value
		}

		//distance calculation
		// distance = |decision value| / |w|

		/*for (int i = 0; i < alpha.length; i++) {
			sumAlpha += alpha[i];
		}*/
		// calculate |w| (|w| =  margin to hyperplane?)
		WAbsVal = Math.sqrt(2 * (si.obj + sumAlpha));
		Log.i("home", "svm.java: si.obj: " + si.obj);
		Log.i("home", "svm;java: sumAlpha: " + sumAlpha);
		Log.i("home", "svm;java: WAbsVal from calculation: " + WAbsVal);
...
public class svm {
	...
	private static void solve_c_svc(svm_problem prob, svm_parameter param,
					...
					double Cp, double Cn)
	{
		Log.i("home", "svm.java: solve_c_svc was called");
		int l = prob.l;
	...
	static decision_function svm_train_one(
		...
		double Cp, double Cn)
	{
		Log.i("home", "svm.java: svm_train_one was called");
		double[] alpha = new double[prob.l];
		Log.i("home", "svm.java: svm_train_one alpha: " + alpha.toString());
		Log.i("home", "svm.java: svm_train_one prob: " + prob.x.toString());
		Log.i("home", "svm.java: svm_train_one prob: " + prob.y.toString());
...
//				svm.info("WARNING: training data in only one class. See README for details.\n");
			...
			int p = 0;
			// it doesn't enter the inner for loop because the number of classes is 1 after one swipe and we start the inner for loop from 1
			for(i=0;i<nr_class;i++)
...
//				Log.i("home", "svm.java: label was added to label array" + label[i]);
...
//			svm.info("Total nSV = "+total_sv+"\n");
	...
	public static void svm_get_labels(svm_model model, int[] label)
	{
		if (model.label != null)
			...
			{label[i] = model.label[i];
				Log.i("home", "svm.java: we have labels");}
		Log.i("home", "svm.java: we have dont have labels");
	...
	{
		//Log.i("home", "svm.java: svm_predict_values is called. label number: " + model.label.length);
		int i;
		...
		else
		{
			...
			//Log.i("home", "svm.java: svm_predict_values returns " + model.label[vote_max_idx]);
    ...
    {
        //Log.i("home", "svm.java: svm_predict is called. label number: " + model.label.length);
        int nr_class = model.nr_class;
        //Log.i("home", "svm.java: number of classes: " + model.nr_class);
        double[] dec_values;
        if(model.param.svm_type == svm_parameter.ONE_CLASS ||
                model.param.svm_type == svm_parameter.EPSILON_SVR ||
                model.param.svm_type == svm_parameter.NU_SVR)
        {
            ...
            //Log.i("home", "svm.java: one class dec_values " + dec_values.length);
        }
        //TODO: was crashing before i added elseif check --> if we get into problem
        // becasue we dont have enough space for dec_value, check here
        else if(nr_class == 1) dec_values = new double[1];
        else
        {	dec_values = new double[nr_class*(nr_class-1)/2];

        }
        double pred_result = svm_predict_values(model, x, dec_values);
        double distance = dec_values[0] * model.label[0] / Solver.WAbsVal;
//        Log.i("home", "svm.java: svm_predict_values: dec_value" + dec_values[0] + "label number" + model.label[0]);
//        Log.i("home", "svm.java: svm_predict_values: dec_value" + (dec_values[0] * model.label[0]));
//        Log.i("home", "svm.java: distance :" + distance);
        return pred_result;
    }

    public static double svm_predict_distance(svm_model model, svm_node[] vector)
    {
        ...
        {	dec_values = new double[nr_class*(nr_class-1)/2];
            //Log.i("home", "svm.java: other class dec_values " + dec_values.length);
            //Log.i("home", "svm.java: no of classes " + nr_class);

        }

        double pred_result = svm_predict_values(model, vector, dec_values);
//        Log.i("home", "svm.java: pred_result: " + pred_result);
        double distance = dec_values[0]* model.label[0]/Solver.WAbsVal;
//		Log.i("home", "svm.java: Solver.WAbsVal: " + Solver.WAbsVal);
        result[0] = pred_result;
        result[1] = distance;
        return result[1];
    }
	public static double svm_predict_probability(svm_model model, svm_node[] x, double[] prob_estimates)
	{
		    ...
		    model.probA!=null && model.probB!=null)
		{
			...
			return svm_predict(model, x); // only called if we dont have the right svm type
	}
